...
const TARGET_COURSES = [
  ...
  { id: "10102958", name: "1151_數值分析_365990" },
  { id: "10102686", name: "1151_界面活性劑原理與應用_364453" },
  { id: "10102684", name: "1151_專題研討_364435" }
];

async function getWsUrl() {
  const res = await fetch(`${CDP_HTTP}/json/list`);
  const list = await res.json();
  const target = list.find(t => t.url && t.url.includes("istudy.ntut.edu.tw/learn/index.php"));
  ...
  async connect() {
    ...
    this.ws.onclose = (e) => {
      ...
      for (const [id, { reject }] of this.pending) {
        reject(new Error(`WebSocket 已關閉 (code: ${e.code}, reason: ${e.reason})`));
      }
      this.pending.clear();
    };
...
async function processCourse(client, course) {
  console.log(`\n========================================`);
  console.log(`開始處理課程：${course.name}`);
  console.log(`========================================`);
  ...
  console.log(`已送出切換指令至課程 ID: ${course.id}...`);
  await sleep(3000);

  // 2. Click "教材及錄影" in mooc_sysbar
  ...
  console.log(`已點擊「教材及錄影」，等待教材樹狀目錄載入...`);

  // 3. Wait for tree.xmlDoc
  let treeReady = false;
  for (let i = 0; i < 20; i++) {
    await sleep(1000);
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    console.log(`\n[${idx + 1}/${items.length}] 處理項目：${item.title}`);
    ...
    try {
      ...
      if (isVideoItem) {
        ...
        if (videoUrl && videoUrl !== "about:blank") {
          console.log(`  ✓ 取得錄影串流網址：${videoUrl.slice(0, 75)}...`);
      ...
      if (pdfUrl) {
        ...
        if (dlResult && dlResult.error) {
          console.log(`  ⚠️ 取得檔案失敗：${dlResult.error}`);
          continue;
        }
        ...
        console.log(`  發現外部資源連結：${externalLink}`);
        ...
        if (url && url !== "about:blank" && !url.includes("SCORM_fetchResource.php")) {
          ...
          console.log(`  [提示] 此項目無可下載之實體檔案。`);
        }
      }
    } catch (itemErr) {
      console.error(`  ⚠️ 處理單元項目時發生異常：${itemErr.message}`);
    }
  }
...
async function run() {
  ...
  console.log(`已成功連線至 Chrome (WS: ${wsUrl})`);
  ...
  for (const course of TARGET_COURSES) {
    try {
      ...
      console.error(`處理課程 ${course.name} 時發生異常：`, courseErr);
    }
  }

  client.close();
  console.log(`\n========================================`);
  console.log(`🎉 全部指定課程的文件與資源下載整理完成！`);
  console.log(`========================================`);
}
